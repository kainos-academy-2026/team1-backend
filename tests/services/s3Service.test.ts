...
describe('S3Service', () => {
	...
	it('returns a presigned upload url and key', async () => {
		mocks.getSignedUrl.mockResolvedValueOnce('https://s3.example.com/upload');
		...
		expect(mocks.s3Client).toHaveBeenCalledWith({ region: 'us-east-1' });
	...
	it('throws when S3_BUCKET_NAME is not set', () => {
		vi.unstubAllEnvs();
		vi.stubEnv('AWS_REGION', 'us-east-1');
