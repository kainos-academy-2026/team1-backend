...
describe('SignupRequestSchema', () => {
	...
	it('rejects a password shorter than 9 characters', () => {
		const result = SignupRequestSchema.safeParse({ ...valid, password: 'Abc123!' });
		expect(result.success).toBe(false);
		if (!result.success) {
	...
	it('rejects a password without an uppercase letter', () => {
		const result = SignupRequestSchema.safeParse({ ...valid, password: 'password123!' });
		expect(result.success).toBe(false);
		if (!result.success) {
			const messages = result.error.issues.map((i) => i.message);
			expect(messages).toContain('Password must include at least one uppercase letter');
		}
	});
	...
	it('rejects a password without a lowercase letter', () => {
		...
		if (!result.success) {
			const messages = result.error.issues.map((i) => i.message);
			expect(messages).toContain('Password must include at least one lowercase letter');
		}
	});
	...
	it('rejects a password without a special character', () => {
		...
		if (!result.success) {
			const messages = result.error.issues.map((i) => i.message);
			expect(messages).toContain('Password must include at least one special character');
		}
	});
